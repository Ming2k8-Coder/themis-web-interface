...
function gNumber(str) {
  const a = globalize("en").numberParser(),
    b = globalize("vi").numberParser();
...
class Log {
  ...
  static filename(user, problem, ext) {
    if (Config.themisCompatible) {
      return path.join(logsPath, `[${user}]`, `[${problem}]${ext}.log`);
    } else {
      return path.join(logsPath, `[${user}][${problem}]${ext}.log`);
    }
  }
...
function getLog(user, problem, ext, callback) {
  ...
  fs.stat(Log.filename(user, problem, ext), (err, stat) => {
    ...
    if (
      ...
      Logs[user][problem + ext].created.getTime() < stat.mtime.getTime()
    ) {
...
// lol cc nha may, es6 hoi met day
//getLog.Log = Log;
//export default Log;
